...
export default function HomeTab({
  ...
  const rawMax = Math.max(
    ...chartData.map(d => {
      ...
      const profit = Math.max(0, d.profit || 0);
      const rto = Math.max(0, d.rtoExpenses || 0);
      const orders = d.orders || 0;

      if (activeChartMetric === "orders") {
        return orders;
      }
      return Math.max(sales, profit + rto, profit, rto);
    }),
    10
  );
  ...
  const getNiceMax = (max: number) => {
    ...
    const step = mag >= 100 ? mag / 2 : mag;
    return Math.ceil(max / step) * step;
  };

  const yAxisMax = Math.max(getNiceMax(rawMax), 40);
        ...
        {!dashboardStats ? (
          ...
          <>
            ...
            <div className={styles.statBox}>
              ...
              <div className={styles.statBottom}>
                <span className={styles.statMainValue}>
                  ₹{(dashboardStats.cardStats?.totalRevenue.value || 0).toLocaleString("en-IN")}
                </span>
            ...
            <div className={styles.statBox}>
              ...
              <div className={styles.statBottom}>
                <span className={styles.statMainValue}>
                  {(dashboardStats.cardStats?.totalOrders.value || 0).toLocaleString()}
                </span>
            ...
            <div className={styles.statBox}>
              ...
              <div className={styles.statBottom}>
                <span className={styles.statMainValue}>
                  ₹{(dashboardStats.cardStats?.netProfit.value || 0).toLocaleString("en-IN")}
                </span>
            ...
            >
              ...
              <div className={styles.statBottom}>
                <span className={styles.statMainValue}>
                  {(dashboardStats.cardStats?.activeCustomers.value || 0).toLocaleString()}
                </span>
        ...
        <div className={styles.chartContainerCard}>
          ...
          <div className={styles.chartCanvasArea} style={{ position: "relative" }}>
            ...
            >
              ...
              {chartData.map((d, i) => {
                ...
                if (activeChartMetric === "profit") {
                  // Full Bar Height represents Revenue (clamped to innerH)
                  const revVal = Math.max(0, d.sales || 0);
                  const revH = Math.min(innerH, (revVal / yAxisMax) * innerH);
                  const revY = Math.max(topPad, topPad + innerH - revH);

                  // Bottom Part: Profit (clamped to revH)
                  const profitVal = Math.max(0, d.profit || 0);
                  const rawProfitH = (profitVal / yAxisMax) * innerH;
                  const profitH = Math.min(revH, rawProfitH);
                  const profitY = topPad + innerH - profitH;

                  // Second Part: RTO Expenses (stacked on top of Profit, strictly inside Revenue Bar)
                  const rtoVal = Math.max(0, d.rtoExpenses || 0);
                  const rawRtoH = (rtoVal / yAxisMax) * innerH;
                  const maxAllowedRtoH = Math.max(0, revH - profitH);
                  const rtoH = Math.min(maxAllowedRtoH, rawRtoH);
                  const rtoY = Math.max(topPad, topPad + innerH - profitH - rtoH);

                  return (
                    <g key={`bar-${i}`} style={{ opacity, transition: 'all 0.2s', cursor: 'pointer' }}>
                      {/* X-Axis Label */}
                      <text
                        x={x + barWidth / 2}
                      ...
                      {/* 1. Full Bar Backdrop: Total Revenue */}
                      {revH > 0 && (
                        <rect
                          x={x}
                          y={revY}
                      ...
                      {/* 2. Second Part: RTO Expenses Segment */}
                      {rtoH > 0 && (
                        <rect
                          x={x}
                          y={rtoY}
                ...
                // Default single bar for Revenue or Orders view
                const val = Math.max(0, d[activeChartMetric] || 0);
                const barH = Math.min(innerH, (val / yAxisMax) * innerH);
                const y = Math.max(topPad, topPad + innerH - barH);
              ...
              {/* Profit Trend Line (Overlay on Revenue View) */}
              ...
              {/* Profit Trend Dots (Overlay on Revenue View) */}
            ...
            {hoveredBarIndex !== null && chartData[hoveredBarIndex] && (
              <div style={{
                ...
                top: `${(topPad + innerH - (((activeChartMetric === "profit" ? (chartData[hoveredBarIndex].sales || 0) : (chartData[hoveredBarIndex][activeChartMetric] || 0))) / yAxisMax) * innerH - 12) / chartH * 100}%`,
                ...
                {/* 1. Revenue View: ONLY 2 values (Revenue & Profit) */}
                {activeChartMetric === 'sales' && (
                  <>
                    <div style={{ fontWeight: '600' }}>
                      Revenue: ₹{(chartData[hoveredBarIndex].sales || 0).toLocaleString('en-IN')}
                    </div>
                    <div style={{ fontWeight: '500', color: '#34d399', fontSize: '12px' }}>
                      Profit: ₹{(chartData[hoveredBarIndex].profit || 0).toLocaleString('en-IN')}
                    </div>
                  </>
                )}

                {/* 2. Profit View: 3 values (Revenue Full Bar, Profit Bottom Part, RTO Expenses 2nd Part) */}
                {activeChartMetric === 'profit' && (
                  <>
                    <div style={{ fontWeight: '600', borderBottom: '1px solid #334155', paddingBottom: '4px', marginBottom: '2px' }}>
                      Revenue (Full Bar): ₹{(chartData[hoveredBarIndex].sales || 0).toLocaleString('en-IN')}
                    </div>
                    <div style={{ fontWeight: '500', color: '#34d399', fontSize: '12px' }}>
                      Profit (Bottom Part): ₹{(chartData[hoveredBarIndex].profit || 0).toLocaleString('en-IN')}
                    </div>
                    <div style={{ fontWeight: '500', color: '#f87171', fontSize: '12px' }}>
                      RTO Expenses (2nd Part): ₹{(chartData[hoveredBarIndex].rtoExpenses || 0).toLocaleString('en-IN')}
                    </div>
                  </>
                )}

                {/* 3. Orders View: 1 value */}
                {activeChartMetric === 'orders' && (
                  <div style={{ fontWeight: '600' }}>
                    Orders: {(chartData[hoveredBarIndex].orders || 0).toLocaleString('en-IN')}
                  </div>
                )}
              </div>
            )}
          ...
          <div className={styles.chartLegendRow}>
            ...
            {activeChartMetric === "profit" && (
              <>
                <span className={styles.legendText}>
                  <span style={{ display: 'inline-block', width: '12px', height: '12px', backgroundColor: '#e2e8f0', borderRadius: '2px', marginRight: '6px', border: '1px solid #cbd5e1', verticalAlign: 'middle' }} />
                  Revenue (Full Bar)
                </span>
                <span className={styles.legendText}>
                  <span style={{ display: 'inline-block', width: '12px', height: '12px', backgroundColor: '#10b981', borderRadius: '2px', marginRight: '6px', verticalAlign: 'middle' }} />
                  Profit (Bottom Part)
                </span>
                <span className={styles.legendText}>
                  <span style={{ display: 'inline-block', width: '12px', height: '12px', backgroundColor: '#ef4444', borderRadius: '2px', marginRight: '6px', verticalAlign: 'middle' }} />
                  RTO Expenses (2nd Part)
                </span>
              </>
            )}
