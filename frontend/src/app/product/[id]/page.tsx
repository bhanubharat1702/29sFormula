                    ...
                    <div className={homeStyles.productCard} style={item.quantity === 0 ? { pointerEvents: "none" } : {}}>
                      <div className={homeStyles.productImageContainer} style={item.quantity === 0 ? { filter: "grayscale(1)", opacity: 0.7 } : {}}>
                        ...
                        <button 
                          ...
                          onClick={(e) => { e.preventDefault(); e.stopPropagation(); const inStockS = item.variants?.find((v: any) => (Number(v.quantity) || 0) > 0)?.size || item.variants?.[0]?.size || item.sizes?.[0] || ""; addToCart(item, inStockS, 1); }}
                        >
